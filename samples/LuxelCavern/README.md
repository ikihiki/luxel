...
- **`LuxelCavern.Core`** (net10.0, 純ロジック): `CavernSim` (プレイヤー物理・収集・敵・トゲ・HP・
  ...
  単体テスト (`tests/GamesSamples/Luxel.GamesSamples.Tests/CavernSimTests` 他) が守る。
- **`LuxelCavern`** (net10.0, WinExe): `CavernRealtimeScene : IGameScene` とscene-owned `IRenderFeature`を
  `LuxelHostBuilder` + `GameLoop` + standard Cadenceで駆動し、`WindowSystem`/`GpuSurface`へ提示する薄い層。
