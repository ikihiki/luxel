...
function decorateDiagnostics(diagnostics, workspace, fallbackFile) {
  return (Array.isArray(diagnostics) ? diagnostics : []).map(diagnostic => {
    const path = diagnostic.path || diagnostic.fileName || fallbackFile.path;
    const owner = workspace.files.find(file => file.path === path) || fallbackFile;
    return { ...diagnostic, workspaceRevision: Number(diagnostic.workspaceRevision ?? workspace.revision), fileId: diagnostic.fileId || owner.id, fileVersion: Number(diagnostic.fileVersion ?? owner.version), path };
  });
}
