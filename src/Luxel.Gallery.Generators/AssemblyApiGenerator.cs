...
/// docs の型 API リファレンス (Reference/Guides/Api2D 等) が実行時にページを組み立てる — reflection なし、
/// 手書き一覧なし。対象アセンブリは GenerateDocumentationFile=true が必要 (無いと説明が空になる)。
/// 説明は参照シンボルからは読めない (csc は参照に DocumentationProvider を付けない) ため、
/// AdditionalFiles で渡された XML doc ファイルを doc-comment ID で引く (Directory.Build.props 参照)。
/// </summary>
...
public sealed class AssemblyApiGenerator : IIncrementalGenerator
{
    ...
    {
        var xmlDocs = context.AdditionalTextsProvider
            .Where(static t => t.Path.EndsWith(".xml", StringComparison.OrdinalIgnoreCase))
            .Collect();
        var source = context.CompilationProvider.Combine(xmlDocs)
            .Select(static (pair, ct) => Collect(pair.Left, pair.Right, ct));
    ...
    private static string? Collect(Compilation compilation,
        System.Collections.Immutable.ImmutableArray<AdditionalText> xmlFiles,
        System.Threading.CancellationToken ct)
    {
        ...
        Dictionary<string, string> docs = ParseXmlDocs(xmlFiles, ct);

        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated/> Luxel.Gallery.Generators.AssemblyApiGenerator");
        sb.AppendLine("#nullable enable");
        sb.AppendLine("namespace Luxel.Gallery.Generated");
        sb.AppendLine("{");
        sb.Append("    internal static class AssemblyApiRegistration_")
          .AppendLine(GeneratedIdentifier.Sanitize(compilation.AssemblyName ?? "Assembly"));
        sb.AppendLine("    {");
    ...
    /// <summary>XML doc ファイル群から member ID → member 要素本文の索引を作る。</summary>
    private static Dictionary<string, string> ParseXmlDocs(
        System.Collections.Immutable.ImmutableArray<AdditionalText> xmlFiles,
        System.Threading.CancellationToken ct)
    {
        var map = new Dictionary<string, string>(StringComparer.Ordinal);
        foreach (AdditionalText file in xmlFiles)
        {
            ct.ThrowIfCancellationRequested();
            string? text = file.GetText(ct)?.ToString();
            if (text is null || !text.Contains("<members>")) continue; // doc ファイル以外の .xml は無視
            foreach (Match m in Regex.Matches(text, @"<member name=""([^""]+)"">(.*?)</member>", RegexOptions.Singleline))
    ...
    private static void EmitType(StringBuilder sb, string ns, INamedTypeSymbol t, Dictionary<string, string> docs)
    {
    ...
    private static string Summary(ISymbol s, Dictionary<string, string> docs)
    {
        string? xml = s.GetDocumentationCommentId() is { } id && docs.TryGetValue(id, out string? fromFile)
            ? fromFile
            : s.GetDocumentationCommentXml(expandIncludes: true);
