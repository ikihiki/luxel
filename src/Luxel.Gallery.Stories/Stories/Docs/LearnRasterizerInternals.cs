...
public static class LearnRasterizerInternals
{
    private static StoryResult Page(string path, string title, string stage, string input, string output,
        string invariant, string correctness, StoryReference live, DocMarkdown source)
        => $$"""
        # {{title}}
        ...
        {{RenderingCourseCatalog.Meta(path, "Advanced", "Implementation reading + Gallery WASM", "GPU compute", "2D描画")}}
    ...
    [Story("Learn/Graphics/2D/Internal/Overview", Order = 18, Toc = true)]
    public static StoryResult Overview() => Page("Learn/Graphics/2D/Internal/Overview", "2D rasterizer pipeline", "全stageの接続",
        ...
        StoryReference.To("Examples/2D/Rasterizer/DispatchLive"), SampleSource("src/Luxel.Graphics.TwoD/GpuDeviceRasterizer2D.cs"));

    [Story("Learn/Graphics/2D/Internal/Flattening", Order = 19, Toc = true)]
    public static StoryResult Flattening() => Page("Learn/Graphics/2D/Internal/Flattening", "Curve flattening", "curve → line segment",
        ...
        StoryReference.To("Examples/2D/Rasterizer/InputPathsLive"), SampleSource("src/Luxel.Graphics.TwoD/PathEncoder.cs"));

    [Story("Learn/Graphics/2D/Internal/SceneEncoding", Order = 20, Toc = true)]
    public static StoryResult SceneEncoding() => Page("Learn/Graphics/2D/Internal/SceneEncoding", "SoA scene encoding", "segments → SoA buffers",
        ...
        StoryReference.To("Examples/2D/Rasterizer/EncodedSceneLive"), SampleSource("src/Luxel.Graphics.TwoD/PathEncoder.cs"));

    [Story("Learn/Graphics/2D/Internal/Abi", Order = 21, Toc = true)]
    public static StoryResult Abi() => Page("Learn/Graphics/2D/Internal/Abi", "C# / Slang ABI", "host layout ↔ shader layout",
        ...
        StoryReference.To("Examples/2D/Rasterizer/EncodedSceneLive"), SampleSource("src/Luxel.Graphics.TwoD/Primitives.cs"));

    [Story("Learn/Graphics/2D/Internal/Bounds", Order = 22, Toc = true)]
    public static StoryResult Bounds() => Page("Learn/Graphics/2D/Internal/Bounds", "Bounds pass", "encoded paths → screen AABB",
        ...
        StoryReference.To("Examples/2D/Rasterizer/BoundsLive"), SampleSource("shaders/raster2d_bounds.slang"));

    [Story("Learn/Graphics/2D/Internal/TileBinning", Order = 23, Toc = true)]
    public static StoryResult TileBinning() => Page("Learn/Graphics/2D/Internal/TileBinning", "16×16 tile binning", "bounds → tile membership",
        ...
        StoryReference.To("Examples/2D/Rasterizer/TileBinsLive"), SampleSource("shaders/raster2d_bin.slang"));

    [Story("Learn/Graphics/2D/Internal/FineRaster", Order = 24, Toc = true)]
    public static StoryResult FineRaster() => Page("Learn/Graphics/2D/Internal/FineRaster", "Fine rasterとcoverage", "tile paths → sample coverage",
        ...
        StoryReference.To("Examples/2D/Rasterizer/CoverageLive"), SampleSource("shaders/raster2d_fine.slang"));

    [Story("Learn/Graphics/2D/Internal/ImagesAndComposite", Order = 25, Toc = true)]
    public static StoryResult ImagesAndComposite() => Page("Learn/Graphics/2D/Internal/ImagesAndComposite", "Image samplingとcomposite", "coverage/image → painter result",
        ...
        StoryReference.To("Examples/2D/Rasterizer/CompositeLive"), SampleSource("shaders/raster2d_fine.slang"));

    [Story("Learn/Graphics/2D/Internal/Dispatch", Order = 26, Toc = true)]
    ...
    [Story("Learn/Graphics/2D/Internal/RetainedUploads", Order = 27, Toc = true)]
    ...
    [Story("Learn/Graphics/2D/Internal/Validation", Order = 28, Toc = true)]
    public static StoryResult Validation() => Page("Learn/Graphics/2D/Internal/Validation", "性能とcorrectnessの検証", "pipeline output → evidence",
        ...
        "shader変更時はSPIR-V/DXIL/WGSL artifactとinputs hashを更新し、WASM live storyもmanifestとbrowser smoke testで検証します。",
        StoryReference.To("Examples/2D/Rasterizer/DispatchLive"), SampleSource("tests/Luxel.Tests/TwoDTests.cs"));
}
