    ...
    private struct Parser(string src)
    {
        ...
        private Val ApplyMethod(Val target, string name, List<Val> args, int pos)
        {
            ...
            Pattern<ControlMap> r = name switch
            {
                ...
                "lpf" or "cutoff" => p.Lpf(FloatPatArg(name, args, pos)),
                "resonance" or "lpq" => p.Resonance(FloatPatArg(name, args, pos)),
                "delay" => p.Delay(FloatPatArg(name, args, pos)),
                "delayfb" or "delayfeedback" => p.DelayFeedback(FloatPatArg(name, args, pos)),
                "delaymix" => p.DelayMix(FloatPatArg(name, args, pos)),
