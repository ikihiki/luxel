...
@media(max-width:820px),(orientation:portrait) and (max-width:1024px){body,body.review-open,body.sidebar-collapsed,body.sidebar-collapsed.review-open{display:block}main{padding:64px 18px 96px}#review-panel{position:fixed;left:0;right:0;bottom:0;top:auto;width:100%;height:auto;max-height:74vh;max-height:74dvh;border-left:0;border-top:1px solid var(--line);border-radius:18px 18px 0 0;box-shadow:0 -12px 32px #0008;padding:24px max(18px,env(safe-area-inset-right)) max(18px,env(safe-area-inset-bottom)) max(18px,env(safe-area-inset-left))}#review-comment{min-height:130px}.review-actions{grid-template-columns:1fr 1fr}}
@media(max-width:520px){.review-actions{grid-template-columns:1fr}.review-actions .primary{grid-column:auto}}
