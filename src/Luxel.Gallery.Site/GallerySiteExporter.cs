...
public static partial class GallerySiteExporter
{
        ...
        string? browserWebGpuRoot = null, string? playgroundBrowserRoot = null)
    {
        ...
        ExportPlayground(output, playgroundBrowserRoot);
    ...
    public static void Validate(string output)
    {
        ...
        RequireRelativeFile(output, "playground.html", "playground route");
...
async function show(){saveCurrentReview();if(location.hash==='#playground'){const r=await fetch('playground.html');content.innerHTML=r.ok?await r.text():'<p>Playgroundを読み込めませんでした。</p>';document.body.classList.remove('runtime-active');setActive('');window.LuxelGalleryPlayground?.bindAll(content);document.title='Playground — Luxel Gallery';content.scrollTo?.(0,0);if(matchMedia('(max-width: 1180px)').matches)setSidebarOpen(false);return}const requested=key();const s=stories.find(x=>x.path===requested||(x.aliases||[]).includes(requested))||stories[0];if(!s)return;const r=await fetch(s.fragment);content.innerHTML=r.ok?await r.text():'<p>ページを読み込めませんでした。</p>';document.body.classList.toggle('runtime-active',!!content.querySelector('.runtime-page'));highlight(content);initializeRuntime(content);setActive(s.path);reveal(s.path);loadReview(s.path);document.title=s.path+' — Luxel Gallery';const section=route().section;if(section)requestAnimationFrame(()=>document.getElementById(section)?.scrollIntoView());else content.scrollTo?.(0,0);if(matchMedia('(max-width: 1180px)').matches)setSidebarOpen(false)}
