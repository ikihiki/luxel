...
public static partial class DocsRenderingLearn
{
    ...
    public static StoryResult Environment(StoryContext ctx)
    {
        ...
        > [!NOTE]
        > `Luxel.Platform`と各platform実装はwindowの作成、event pump、clipboard、IME、低レベル入力をサポートします。通常のFrameworkアプリでは`Luxel.Framework.UI`の構成にdeviceとsurfaceの自動管理を任せられます。以下はgraphics APIを直接組み立てる低水準向けの例です。
    ...
    public static StoryResult PipelineState(StoryContext ctx)
    {
        ...
        - **一部だけ欠ける**: viewport/scissorをtarget size内に戻し、resize後はcolor/depth targetと矩形を同じsize基準で更新する。
        """;
    }
