...
/// <summary>
/// Creates GPU objects as owned <see cref="ResourceScope"/> resources. Local keys are
/// qualified by the scope owner, so callers can use stable component-local names without
/// moving graphics descriptors into <c>Luxel.Resources</c>.
/// </summary>
public static class ResourceScopeGpuExtensions
{
        ...
        where T : unmanaged
    {
        ...
        return scope.CreateBuffer(device, localKey, sizeInBytes, kind);
    }
}
