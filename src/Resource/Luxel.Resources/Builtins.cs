...
/// <summary>Platform の読取抽象を file:// resource source として直接利用する adapter。</summary>
public sealed class PlatformFileSource(IPlatformFileReader reader) : IResourceSource
{
    public IEnumerable<string> Schemes => ["file", ""];
    public Task<byte[]> ReadAsync(ResourceUri uri, LoadContext ctx)
        => reader.ReadAllBytesAsync(uri.Path, ctx.Token);

    public IReloadToken? Watch(ResourceUri uri, Action onChanged)
    {
        if (reader is not IPlatformFileSystem fileSystem) return null;
        IDisposable? token = fileSystem.Watch(uri.Path, onChanged);
        return token is null ? null : new PlatformReloadToken(token);
    }

    private sealed class PlatformReloadToken(IDisposable token) : IReloadToken
    {
