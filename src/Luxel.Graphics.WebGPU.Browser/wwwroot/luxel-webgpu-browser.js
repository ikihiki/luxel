...
function configureSurface(surface) { if (surface.canvas.width !== surface.width) surface.canvas.width = surface.width; if (surface.canvas.height !== surface.height) surface.canvas.height = surface.height; if (surface.width && surface.height) surface.context.configure({ device:surface.backend.device, format:surface.format, usage:GPUTextureUsage.RENDER_ATTACHMENT|GPUTextureUsage.COPY_SRC, alphaMode:"premultiplied" }); else surface.context.unconfigure(); }
