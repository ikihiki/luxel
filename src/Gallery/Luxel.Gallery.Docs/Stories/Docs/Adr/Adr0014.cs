...
public static partial class DocsAdr
{
    ...
    public static StoryResult Adr0014(StoryContext ctx) => $$"""
        ...
        Workbench ([ADR-0010](story:Internals/ADR/Adr0010)) のシェルに必要な**内容/レイアウト系コントロール**が未実装です。調査の結果、次が「無い」と判明しました: 複数ドキュメントの `DocumentTabs`、ドッキングを描く `DockHost`、型/ECS を反映する `PropertyGrid`/Inspector、`StatusBar`、VFS 連動の `AssetBrowser`。
        ...
        - **`DocumentTabs`** — 複数ドキュメントのタブ帯 { ダーティ点, × 閉じ, オーバーフロー, D&D 並べ替え }。既存 `Tabs` を汎用化。D&D は [ADR-0011](story:Internals/ADR/Adr0011) が前提
        - **`DockHost`** — `DockTree` ([ADR-0010](story:Internals/ADR/Adr0010)) を描く container。ペイン分割 (`Splitter`)・ドラッグの**ドロップゾーン**・窓内フローティング (Overlay 層、[ADR-0007](story:Internals/ADR/Adr0007))
        ...
        - **`AssetBrowser` / FileTree** — `TreeView` を VFS ([ADR-0012](story:Internals/ADR/Adr0012) の Write を含む `IDocumentStore`) に結線 (+サムネイルグリッド)

        **コマンド駆動サーフェス** (MenuBar / コマンドパレット / ツールバー) は [ADR-0013](story:Internals/ADR/Adr0013) 側に置き、この ADR は内容/レイアウト系に集中します。ヒエラルキー/アウトライナは `TreeView` 流用で新規不要です。
        ...
        - ⚠️ `DockHost` の D&D/ドロップゾーンは実装が重く、[ADR-0011](story:Internals/ADR/Adr0011) と**端レイアウトの golden 検証**が要る
