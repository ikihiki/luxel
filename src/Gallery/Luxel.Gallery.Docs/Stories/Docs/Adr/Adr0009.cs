...
{
    [Story]
    public static StoryResult Adr0009(StoryContext ctx) => $$"""
        ...
        - **`Luxel.Document` (テキスト新スタック、[ADR-0006](story:Internals/ADR/Adr0006))** — こちらは**アーキテクチャの手本**になる: 不変状態 + Transaction + 純射影ジオメトリ + 薄い view が、canvas 非依存で headless テスト可能という Luxel の強みをそのまま出せている

        また描画・入力の素材は既に揃っています — `Scene2D.CubicTo` (接続線のベジェ)・`AddHit` のドラッグフック・`Affine2D` コンテナ変換 (pan/zoom)・`PopupPlacer` ([ADR-0007](story:Internals/ADR/Adr0007) のノード追加パレット)・`DiagramLayout` (自動整列)。

        ## Decision

        テキスト新スタック ([ADR-0006](story:Internals/ADR/Adr0006)) と**同じ骨格**で、canvas 非依存の新プロジェクト **`Luxel.NodeGraph`** を新規に作り、その上に薄い view `NodeGraphView` ([UiComponent]) を載せます。既存の Diagram / Animation.Graph は触らず並置します (Diagram の配置アルゴリズムだけ参照)。
        ...
        - ⚠️ PointerEvent に修飾キーが無い制約で、追加クリック選択の操作感はフレームワーク拡張まで範囲選択で代替 (マルチカーソル / [ADR-0008](story:Internals/ADR/Adr0008) と共有の穴)
