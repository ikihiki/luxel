...
public sealed class ComponentStoryGenerator : IIncrementalGenerator
{
    ...
    private static void Emit(SourceProductionContext context, ImmutableArray<StoryModel?> models, string assemblyName)
    {
        ...
        valid.Sort(static (left, right) => string.CompareOrdinal(left.Path, right.Path));

        var source = new StringBuilder();
        source.AppendLine("// <auto-generated/> Luxel.Gallery.Generators.ComponentStoryGenerator");
    ...
    {
        source.AppendLine();
        source.Append("        private static readonly global::Luxel.Gallery.StoryArgDefinition[] Args_").Append(index).AppendLine(" =");
        source.AppendLine("        [");
        foreach (ArgModel arg in story.Args.OrderBy(static arg => arg.Order).ThenBy(static arg => arg.Name, StringComparer.Ordinal))
        {
            source.Append("            global::Luxel.Gallery.StoryArgDefinition.Create<").Append(arg.Type).Append(">(")
    ...
    private static void EmitBuilder(StringBuilder source, StoryModel story, int index)
    {
        ...
        for (int i = 0; i < story.Args.Count; i++)
        {
            ...
            if (arg.Description is not null || arg.Order != 1000 || arg.Min.HasValue || arg.Max.HasValue || arg.Step.HasValue)
            {
                source.Append(", new global::Luxel.Gallery.StoryArgOptions<").Append(arg.Type).Append("> { ");
