...
public sealed class Gallery2DStoryTests(BrowserFixture fixture) : GalleryBrowserTest(fixture)
{
    ...
    [Fact]
    public Task ShowsLoadingProgressBeforeWebAssemblyStarts() => RunAsync(nameof(ShowsLoadingProgressBeforeWebAssemblyStarts), async page =>
    {
    ...
    [Fact]
    public Task RendersMarkdownOverviewsWithNavigationAndSearch() => RunAsync(nameof(RendersMarkdownOverviewsWithNavigationAndSearch), async page =>
    {
        ...
        await EventuallyAsync(() => main.EvaluateAsync<bool>("element => element.scrollHeight > element.clientHeight"));
        ...
        await EventuallyAsync(async () => await main.EvaluateAsync<float>("element => element.scrollTop") > 0);
        await EventuallyAsync(async () => Math.Abs(await page.Locator(".story-toolbar").EvaluateAsync<float>("element => element.getBoundingClientRect().top") - toolbarTop) < 1);
    ...
    [Fact]
    public Task ExposesArgsOutputSourceAndResizablePreview() => RunAsync(nameof(ExposesArgsOutputSourceAndResizablePreview), async page =>
    {
        ...
        await EventuallyAsync(async () => await runtime.Locator("html").EvaluateAsync<int>("() => globalThis.luxelBrowserState?.count") == 7);
    ...
    [Fact]
    public Task CompactStoriesExposeInteractivePanels() => RunAsync(nameof(CompactStoriesExposeInteractivePanels), async page =>
    {
        const string story = "Controls/Button/Counter";
        ...
        await count.BlurAsync();
        await EventuallyAsync(async () => await page.EvaluateAsync<int>("() => globalThis.luxelBrowserState?.count") == 4);
        await page.GetByRole(AriaRole.Tab, new() { Name = "出力" }).ClickAsync();
        await Expect(page.Locator(".output-list")).ToContainTextAsync("引数を変更しました");
        await page.GetByRole(AriaRole.Tab, new() { Name = "ソース" }).ClickAsync();
        await Expect(page.Locator(".story-source")).ToContainTextAsync("ButtonCounter");
    ...
    [Fact]
    public Task EmbeddedWidgetStoriesRemainCanvasOnly() => RunAsync(nameof(EmbeddedWidgetStoriesRemainCanvasOnly), async page =>
    {
        await page.GotoAsync(StoryPath("Controls/Button/Counter"));
    ...
    public Task RendersAnimationStory(string story) => RunAsync(story, async page =>
    {
        var failures = CollectFailures(page);
        await page.GotoAsync(StoryPath(story));
        await ExpectRuntimeStoryAsync(page, story, webGpu: true, statusText: true);
    ...
    [Fact]
    public Task StateMachineRespondsToPressAndDoneTriggers() => RunAsync(nameof(StateMachineRespondsToPressAndDoneTriggers), async page =>
    {
        var failures = CollectFailures(page);
        ...
        var press = await FindCanvasWidgetAsync(page, "press");
        ...
        await EventuallyAsync(async () => await page.EvaluateAsync<int>("() => globalThis.luxelBrowserState.renderRevision") > pressRevision + 2);
        ...
        var done = await FindCanvasWidgetAsync(page, "done");
        await page.Mouse.ClickAsync(done.X + done.Width / 2, done.Y + done.Height / 2);
        await EventuallyAsync(() => page.EvaluateAsync<bool>("() => globalThis.luxelBrowserState.events.some(entry => String(entry.message || entry).includes('done'))"));
        Assert.Empty(failures.ConsoleErrors);
        Assert.Empty(failures.PageErrors);
    });

    private Task VerifyGpuStoryAsync(string story, bool gpuView = false) => RunAsync(story, async page =>
    {
        var failures = CollectFailures(page);
        await page.GotoAsync(StoryPath(story));
        await ExpectRuntimeStoryAsync(page, story, webGpu: true, statusText: true);
    ...
    private static Task ExpectGpuViewReadyAsync(IPage page) => EventuallyAsync(async () =>
