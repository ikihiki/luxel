...
test('Web Audio bus, spatial, and queued-buffer controls update observable state', async ({ page }) => {
  ...
  await page.goto(`/samples/webgpu-browser/?story=${encodeURIComponent('Examples/Audio/Buses')}`);
  ...
  await page.goto(`/samples/webgpu-browser/?story=${encodeURIComponent('Examples/Audio/SpatialAttenuation')}`);
  ...
  await page.goto(`/samples/webgpu-browser/?story=${encodeURIComponent('Examples/Audio/StreamingQueue')}`);
