...
- [ ] **Q09**: [16 パーティクル](16-particle-system.md) — コア + .TwoD + .ThreeD (ビルボード) まで全部 (着手中: 2026-07-06 — **コア + .TwoD + .ThreeD ビルボード + JSON 資産 完了** (billboard.slang + ParticleBillboards + Spherical 放出 + Demos/3D/Particles vk golden、ParticleConfigJson 往復 + ステップ、単体テスト計 22)。**残り (最終セッション): ParticleView [UiComponent] + BreakoutStory の dogfooding 置換**。詳細は 16 MD 進捗節)
