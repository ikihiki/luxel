...
- [ ] **Q11**: [01 ScriptSystem + csx hot reload](01-scripting-scriptsystem-hot-reload.md) — capstone ① の敵 AI を csx で書くための土台 (着手中: 2026-07-06 — **コア完了**: 新プロジェクト Luxel.Scripting.Framework (ScriptSystem + ScriptSystems 記述子 + ScriptGameGlobals) + IScriptSource(Memory/File)。安定ラッパ Attach + Reload (失敗時は旧維持+診断、実行時例外は捕捉) + 単体テスト 6 + Docs/Scripting 節。**残り: Demos/Scripting/HotReload デモ (CodeEditor + 実行シーン + play) + GalleryServices に game globals 用 ScriptHost 登録 + golden**。詳細は 01 MD 進捗節)
