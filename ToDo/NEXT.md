...
- [ ] **Q17**: [09 glTF skin/morph](09-gltf-skin-morph.md) — 大。**skin スライス完了 (2026-07-07)、morph 残** (着手中)。調査で判明: skin はデータ層 (AssetSkin/JOINTS_0/WEIGHTS_0 パース、SceneBuilder の SkinnedVertex アップロード、SkinningSystem の joint 行列 = InverseBind×jointWorld、`scene_pbr_skinned.slang`) が**既に scaffold 済み・未結線**。**実装 (skin)**: `scene_pbr_skinned.slang` を SceneInstanceData 形式 (world+baseColor) + joint バッファに整え (matIdx/material 版から差替、glTF 規約で skinned node 変換は無視=instance world 恒等)、デモ `Demos/3D/GltfSkinned` (RiggedSimple.glb をアニメ固定時刻で sample→伝播→SkinningSystem→joint upload→GPU 頂点スキニング描画、曲がった棒、vk/dx golden)。単体テスト `SkinningSystemTests` 3 (行列=InverseBind×jointWorld / バインドポーズ=単位 / joint 欠落フォールバック)、全 818 passed、e2e 69/69 diff 0・vk/dx 一致。Docs/Motion 更新。**残 = morph** (loader が primitive.targets/mesh.weights を未パース = greenfield、weights channel は SceneAnimationPlayer が skip 中、morph シェーダ無し)。**09 MD は morph 完了まで残す**。
