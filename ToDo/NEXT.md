...
- [ ] **Q09**: [16 パーティクル](16-particle-system.md) — コア + .TwoD + .ThreeD (ビルボード) まで全部 (着手中: 2026-07-06 — **コア Luxel.Particles + .TwoD 完了** (ParticleSystem/Buffer/Simulator/Value/Color/Config + Xorshift64 + ParticleNode、単体テスト 18、デモ Demos/TwoD/Particles vk golden + Docs)。**残り (次セッション): .ThreeD ビルボード + Slang シェーダ / JSON 資産化 (ParticleConfig FromJson + リソースステップ) / ParticleView [UiComponent] / BreakoutStory の dogfooding 置換 / Demos/ThreeD/Particles**。詳細は 16 MD 進捗節)
