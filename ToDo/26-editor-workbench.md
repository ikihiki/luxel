...
- **S(A2)** Markdown 装飾プロバイダ + widget リゾルバ (`Luxel.Controls`) — パーサ → Line/Block/Mark/LinePrefix + widget。内容プロセッサ (Diagram/MathText/Highlight.TextMate) を **widget コンテンツ**として再利用し、`Luxel.Document` 依存の共有ブロック/フォーマット型をここへ移設 (or 中立化)。**read-only 描画モード** = Docs レンダラ。
  - **S(A2) 一部 済** (2026-07-09): `MarkdownDecorations.Build` (見出し/太字/斜体/インラインコード = font-variant Mark、記法マーカ淡色) + **block-level** (引用 = BlockDecoration バー+インデント / 箇条書き・番号リスト = マーカ淡色 / コードフェンス = LineDecoration 行背景 + Mono) + `MarkdownProvider` (`IDecorationProvider`) + `FontVariant.Mono`。単体 9 + story Markdown (vk/dx golden)、e2e 98/98。既存の Block/Line/LinePrefix 装飾で実現 = エンジン改修なし。
  - **ブロック widget 済** (2026-07-09): `BlockWidgetDecoration` (複数ソース行占有) — geometry が先頭行=宣言高さ全幅スロット・残り=高さ0畳み (行↔ソース 1:1 保持)。単体 3 + story BlockWidget (vk/dx golden)。表/mermaid/数式/埋め込みの土台。
  - **残**: 内容プロセッサ (Diagram/MathText/Highlight.TextMate) を block widget コンテンツ化 + read-only モード明示。
