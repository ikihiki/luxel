...
### GE-0 — プロジェクト/シーンモデル (純ロジック) — ✅ 完了 (2026-07-10, Q45)

実装済み: `Luxel.SceneEdit` + ADR-0015 (Accepted)。設計判断の追加分: **値は形ベース** (SceneValue = Bool/Number/Text/Vec2/3/4/Raw、型付きの意味はスキーマの解釈 — 未知保全が構造的に成立)、コンポーネントは**エンティティ内で型ごとに 1 個**、フィールドは名前順ソートで正規化 (決定的 JSON)、フィールド名 "type" は予約。詳細は NEXT.md Q45 の完了ログと ADR-0015。
